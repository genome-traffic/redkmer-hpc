...
awk '{print $0, ($8+$9+$10),($14-($8+$9+$10))}' $CWD/kmers/kmer_results.txt > $TMPDIR/tmpfile; mv $TMPDIR/tmpfile $CWD/kmers/kmer_results.txt
...
#Add column header
awk 'BEGIN {print "kmer_id\tseq\tfemale\tmale\tCQ\tsum\thits_X\thits_A\thits_Y\thits_GA\thits_sum\tperchitsX\thits_threshold\tsum_offtargets\tofftargets\tdegen_targets"} {print}' $CWD/kmers/kmer_results.txt > $TMPDIR/tmpfile; mv $TMPDIR/tmpfile $CWD/kmers/kmer_results.txt
